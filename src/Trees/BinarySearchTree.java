...
public class BinarySearchTree<T> extends AbstractSet<T>
{
    ...
    public double averageDepth() {
		return ((double) totalDepth(root, 0)) / ((double) size);
    }

    private int totalDepth(Node<T> node, int curDepth) {
    	if(node == null) { return 0; }
    	return curDepth + totalDepth(node.left, curDepth+1) + totalDepth(node.right, curDepth+1);
    }
